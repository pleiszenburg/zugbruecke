...
def _download(_down_url):
    try:
        ...
        try:
            ...
            raise SystemExit(
                'SSL/TSL has issues - please install "certifi" and try again', e.args[0]
            )
...
class env_class:
    ...
    def _init_path_dict(self):
        ...
        stdlibzip_path = os.path.join(
            self._p["pythonprefix"], "python%s.zip" % version_string
        )
        # pth filename (library path)
        pth_path = os.path.join(
            self._p["pythonprefix"], "python%s._pth" % version_string
        )
    ...
    def setup_python(self, overwrite=False):
        ...
        pyurl = "https://www.python.org/ftp/python/%s/%s" % (
            fix_special_version(self._p["pythonversion"]),
            pyarchive,
        )
    ...
    def setup_coverage_activate(self):
        ...
        siteconfig_path = os.path.join(
            self._path_dict["sitepackages"], "sitecustomize.py"
        )
        siteconfig_cnt = ""
