...
class routine_client_class:
    ...
    def __call__(self, *args):
        ...
        # Log status
        self.log.out(
            '[routine-client] Trying to call routine "%s" in DLL file "%s" ...'
            % (self.name, self.dll.name)
        )
        ...
        # Log status
        self.log.out(
            '[routine-client] ... parameters are "%r". Packing and pushing to server ...'
            % (args,)
        )
        ...
        return_dict = self.__handle_call_on_server__(
            self.data.arg_list_pack(args, self.argtypes_d, self.dll.calling_convention),
            mem_package_list,
        )
        ...
        self.data.arg_list_sync(
            args,
            self.data.arg_list_unpack(
                return_dict["args"], self.argtypes_d, self.dll.calling_convention
            ),
            self.argtypes_d,
        )
        ...
        return_value = self.data.return_msg_unpack(
            return_dict["return_value"], self.restype_d
        )
        ...
        self.data.client_unpack_memory_list(
            args, return_value, return_dict["memory"], self.memsync_d
        )
